...
export default function EditGameResultPage() {
  ...
  const fetchGame = async () => {
    try {
      ...
      const gameDoc = await getDoc(doc(db, 'gameResults', gameId));

      if (gameDoc.exists()) {
        const data = gameDoc.data();
        setOurTeamName(data.ourTeamName || 'ポストン');
        ...
        // イニングスコア
        if (data.inningScores && data.inningScores.length > 0) {
          setInningScores(
            data.inningScores.map((s: { inning: number; ourScore: number; opponentScore: number }) => ({
              inning: s.inning,
              ourScore: s.ourScore ?? null,
              opponentScore: s.opponentScore ?? null,
            }))
          );
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'gameResults', gameId), {
    ...
    <div className="min-h-screen bg-gradient-to-b from-primary-50 to-white">
      ...
      <header className="bg-primary-500 text-white p-4 md:p-6 shadow-lg">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <h1 className="text-2xl-mobile md:text-3xl font-bold">✏️ 試合結果を編集</h1>
          <Link
              ...
              <table className="w-full text-center border-collapse" style={{ minWidth: '520px' }}>
                <thead>
                  <tr className="bg-primary-500 text-white">
                    ...
                    {inningScores.map(s => (
                      <th key={s.inning} className="px-2 py-3 text-lg w-12">{s.inning}</th>
                    ))}
